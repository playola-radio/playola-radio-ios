...
**Test execution note:** Run `xcodebuild test` with destination `'platform=iOS Simulator,name=iPhone 17 Pro,OS=26.4.1'` (iPhone 15 isn't installed in this environment). For new tests, scope to the suite (e.g. `-only-testing:PlayolaRadioTests/PresetTests`). Per the codebase memory `feedback_run_tests_not_just_build.md`: `build-for-testing` only compiles — actually run tests before claiming pass.

## Implementer conventions (MANDATORY — apply to every task)

Before writing or editing any Swift code, every implementer MUST invoke the Point-Free Workflow skills that apply to the task. Reference list:

- **`pfw-dependencies`** — when editing `APIClient`, anything `@Dependency`-related, or `@DependencyClient` structs.
- **`pfw-observable-models`** — when editing `StationListModel` or any other `@Observable` model.
- **`pfw-testing`** + **`pfw-custom-dump`** — when writing or editing tests.
- **`pfw-sharing`** — when adding or editing `@Shared` keys.
- **`pfw-identified-collections`** — when working with `IdentifiedArrayOf<…>`.
- **`pfw-modern-swiftui`** — for any SwiftUI view work (Tasks 15–22).
- **`pfw-case-paths`** — when asserting on enum cases with associated values in tests.

**Test assertion API (applies to every test code block below):**

- Prefer **`expectNoDifference(lhs, rhs)`** over `#expect(lhs == rhs)` for value-type equality of structs or collections — gives a pretty diff on failure.
- Use **`expectDifference(model.state) { action } changes: { mutation }`** for tests that call a method and verify state change (most of the mutation tests in Tasks 7, 10–13). Mutate the smallest field possible. Insert/remove for collections; reassign for scalars.
- Keep raw `#expect` for boolean predicates and individual property reads.
- Import: `import CustomDump` in any test file that uses these. (Already imported in `SongSeedTests.swift` as a reference.)
- Per the `pfw-custom-dump` skill: do NOT use legacy `XCTAssertNoDifference` / `XCTAssertDifference`.

The test code blocks below are written with `#expect` for readability; when implementing, translate to `expectNoDifference` / `expectDifference` per the rules above.
