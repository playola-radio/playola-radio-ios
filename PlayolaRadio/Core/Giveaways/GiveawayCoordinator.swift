...
final class GiveawayCoordinator {
  ...
  func start() {
    guard GiveawayFeature.isLiveDataEnabled, feedPollTask == nil else { return }
  ...
  func reconcile() async {
    guard let jwt = auth.jwt, let stationId = currentPlayolaStationId else {
      clearActiveAndArm()
      return
    }
  ...
  func revealEvent(jwt: String, eventId: String, expectedStationId: String) async {
    do {
      ...
      guard currentPlayolaStationId == expectedStationId else { return }
    ...
    async
  {
    ...
    guard let opensAt = event.opensAt, let serverTime = event.serverTime else { return }
