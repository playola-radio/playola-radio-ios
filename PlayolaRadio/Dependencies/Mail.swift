...
class EmailService: NSObject, MFMailComposeViewControllerDelegate {
  public static func createEmailUrl(
    ...
    var sparkUrlStr =
      "readdle-spark://compose?recipient=\(to)&subject=\(subjectEncoded)"
    ...
    if let gmailUrl = URL(string: gmailUrlStr),
      UIApplication.shared.canOpenURL(gmailUrl)
    {
      return gmailUrl
    } else if let outlookUrl = URL(string: outlookUrlStr),
      UIApplication.shared.canOpenURL(outlookUrl)
    {
      return outlookUrl
    } else if let yahooMail = URL(string: yahooMailStr),
      UIApplication.shared.canOpenURL(yahooMail)
    {
      return yahooMail
    } else if let sparkUrl = URL(string: sparkUrlStr),
