...
- **`POST /stations/:stationId/liveShow` `{ type, audioBlockIds: [~10] }`** — under the station lock, in one transaction: find the next **safe** boundary (a song boundary ≥ the 2-min horizon that doesn't collide with an already-scheduled Episode/LQ airing — reuse the scheduler's existing hard-boundary handling), **create the `liveShows` row first** (`endingSpinId = NULL`) so the `spins.liveShowId` FK resolves, then batch-insert the explicit spins + **3 trailing fillers** tagged with that `liveShowId` (one operation, not N committed edits — `audioBlockScheduling.ts:113`). Enqueue the go-live push job **after the transaction commits** (never against an uncommitted/rolled-back write). This create-then-insert order avoids needing a deferred FK. Returns `liveShowId` + derived `scheduledStartsAt`/`scheduledEndsAt`.
