...
Final review: Claude returned PASS with no blocking findings. Retained existing page-lifetime cancellation rather than adding persistence or continuing unscheduled work after abandonment; its non-blocking note about unscheduled audio on exit is outside the owner's in-page ordering request. A final local regression exposed outro acceptance being rejected if an earlier insert started while the recorder was open. Acceptance now checks permission to queue audio, which permits an in-flight insert, rather than permission to initiate End Show. The suspended-insert regression fails before the correction and passes afterward. Final permanent run: 191 tests pass across all six suites, strict lint and full format check clean. No temporary rendering tests remain.

## Independent scheduling with local recording positions (2026-09-21)

Owner supersedes the FIFO behavior: uploading voicetracks retain local desired positions while later songs schedule and move immediately. Remove “Waiting to schedule” copy. Preserve Broadcast upload/Scheduling visuals, dedicated AMA layout, hidden filler, and terminal outro.

Architecture: store predecessor row IDs only for pending entries, ordered nearest-first as fallbacks when predecessors disappear. These are presentation IDs, never fabricated Spin values. Render a single merged list from the authoritative saved rows plus pending rows at their anchored positions. Snapshot pending predecessors on acceptance and after user moves. When a later item schedules first, replace references to its local ID with the returned new spin ID; earlier recordings retain their intended position before it. Re-resolve every insertion against the latest merged list and existing beforeSpinId API, whose shared implementation resolves the real predecessor. Clamp positions forward beyond the locked prefix and before replaceable filler. No safe target leaves that item retryable. Actual server order remains authoritative; local anchors only position unscheduled rows.

Ready-item selection skips uploading and individually failed items; failed IDs become a Set. The existing atomic MainActor drain guard and repeated current-state selection serialize API calls without blocking other ready items. Explicitly require the outro be the only remaining pending entry before endLiveShow. Merged moves operate in display-row coordinates, remap saved spins for the existing move API, and update pending anchors locally; moves across only a pending row need no server request. Pending outro stays last. Roll back anchors if a server move fails. No additional scheduler, persistence, poller, or API shape changes.

Consultation dispositions: adopted predecessor anchors, per-item failures, forward fallback at the two-minute lock, merged-list index translation, and explicit outro gate. Kept existing beforeSpinId translation instead of rewriting the shared insertion API. A nearest-first predecessor list supplies deletion fallback; the displayed position is clamped forward to the same safe boundary used for insertion. Repeated live-state ready selection handles uploads completed during an await without a second drain-request flag or an index iterator across suspension. Server origin/develop 365e2cc1 in the georgetown checkout confirms ordinary inserted spins inside retained show content are tagged and repaired by classifyLiveShowPlacement; wire contracts unchanged.

Verify: reproduce later-song blocking first; reverse upload completion, real/local moves across placeholders, move rollback, deletion/expiration fallback, independent failed retries, outro waiting for unfinished audio, and all pre-existing touched suites. Render mixed saved/pending rows and run format/lint plus one light final Claude review per handoff.

Verification: 199 permanent regression tests plus one temporary two-state simulator render test pass across seven suites (placement, AMA presentation/page, Broadcast, Shows, navigation, recorder). Renders verify an uploading voicetrack interleaved before an already-scheduled later song and moved locally to an earlier safe slot; artifacts are .context/ama-position-validation. Temporary render code removed. Strict lint (362 source files) and full format check pass. Tests cover immediate song insert, reverse completion, local-only pending moves, saved-song moves across placeholders, failed-move rollback, deleted predecessor fallback, forward clamping at the safety boundary, per-item retry without blocking songs, and replacement during a move. Shared move returns success for local anchor rollback, discards stale responses on show replacement, and clears obsolete optimistic ordering when show identity changes. Grouped move semantics remain intact.

Final Claude review returned PASS with a non-blocking suggestion to expose a global retry through End Show when an earlier recording failed. Kept the approved row-local recovery: the failed recording visibly offers Retry scheduling and discard, either of which lets the terminal outro proceed. The integration test's direct retryAddingAudio call occurs during an in-flight insert and is deliberately a no-op, not hidden recovery. Expanded the failed-insert test to queue a ready outro and exercise the actual failed-row retry action; it inserts the recording and submits the outro without End Show being tapped again. This preserves the requested per-item flow without adding waiting explanations or a second retry surface. Full reversed predecessors on append represent the whole preceding playlist; moved entries capture just their new prefix, so the two forms implement the same invariant.

## Persist the prefilled opening playlist (2026-09-21)

Owner now requests that the pre-start playlist survive app restarts. Preserve the ordered intro, songs, and completed voicetracks, scoped to the signed-in host and station. Leaving setup must retain this draft. An accepted Start Show clears the saved draft while the in-memory opener continues to supply provisional live rows until the first schedule refresh; a failed start retains the draft. Restore only setup content, not active-show pending scheduling state. Interrupted setup recordings should retain their source and resume through the existing upload service after the schedule confirms setup is still appropriate. No new server endpoint, scheduler, or screen design.

Implementation: one file-backed Shared dictionary indexed by captured owner ID and station ID stores Codable opening items. A guarded collection observer saves structural/readiness changes; pending progress is normalized so upload ticks do not write the draft repeatedly. Restore resets unfinished uploads and rebases their source path to Application Support. A small recording-file dependency moves accepted setup recordings out of temporary storage; cancellation preserves them, and reopening setup resumes through the existing task dictionary after a successful schedule load. Completed uploads explicitly await Shared.save before deleting their source. Successful Start clears the durable draft immediately; finding an existing show also clears consumed setup content. No draft expiration or cross-station overwriting.

Architecture consultation: accepted the fileStorage approach. Rejected automatic 24-hour expiry and dropping interrupted recordings because they would lose the owner's preparation. Claude agreed on clearing after accepted Start after reconsidering the already-existing schedule recovery path. Its warning that Sharing has no flush API was checked against the resolved package: Shared.save() exists and FileStorageKey's userInitiated save bypasses the throttle. Tests cover the optional-chained completion mutation persisting via didSet. Orphan sweeping and new retry UI remain outside this focused change; referenced pending recordings are deliberately retained for an unfinished draft.

Verification: 210 tests pass across nine suites: draft persistence, AMA page/presentation/placement, Broadcast, Shows, navigation, recorder, and upload service. A fresh dependency/cache scope reloads encoded draft data and completed voice readiness; separate coverage checks account/station isolation, failed-start retention, accepted-start removal while provisional rows remain, interrupted upload resumption in order, durable source movement/container rebasing, and deferring recovery until a successful non-live schedule check. The existing Back test now expects cancellation to preserve the setup recording. Strict lint (364 files), format check, and git diff --check pass. No wire contract changes: startLiveShow still POSTs the ordered audioBlockIds and clears the draft only after its decoded success response.

Final light Claude review: PASS. Its two non-blocking notes are retained by design: the task-cancellation guard also rejects stale live upload callbacks, which serves the existing discard/replacement behavior and is covered by the passing live-queue regressions; unfinished sources remain referenced by account/station drafts across sign-out, serving the requested durable preparation rather than expiring or deleting it. No sign-out wipe or orphan-sweep subsystem added. The review independently verified the resolved Sharing save/flush ordering. No product changes were needed after review.

## Keep unchanged airtimes visible during moves (2026-09-21)

Owner requests that moving a spin on Broadcast or the show editor only show airtime spinners from the first changed position onward. The shared move handler now compares the original IDs with the optimistic order after expanding grouped moves, and marks only that suffix as rescheduling. An unchanged order marks no rows. Existing API calls, locks, rollback, and spinner cleanup are unchanged. Updated suspended-request tests cover upward/downward moves, grouped moves, unchanged drops, and show-editor prefix airtimes during success/failure. All 137 tests across Broadcast and the three AMA page/presentation/placement suites pass; strict lint, format check, and diff whitespace check pass. This small presentation-only correction uses the trivial-change exception for external review.

## Forward Shows directly to an active show (2026-09-21)

Owner requests that Shows skip choosing a show type whenever a show is already in progress. On appearance, keep the chooser hidden/noninteractive and load the station's extended schedule using a fresh AMA model's existing Broadcast loader. After verifying the task is not cancelled and this exact Shows model is still the current navigation entry, call the existing schedulePlaybackChanged detection. An active show replaces only the current Shows entry, retaining its parent Back destination and the already-loaded schedule. No active show unlocks the existing chooser. A failed check keeps it locked and provides inline Retry. Reappearing performs a fresh check; an in-flight guard prevents overlapping appearance/retry work.

Architecture consultation adopted preloading and replacing the top entry rather than resetting the navigation stack or pushing onto the chooser. Refined full AMA viewAppeared to loadSchedule plus schedulePlaybackChanged: the check must not subscribe to live updates, load listener metadata, or resume draft uploads while the chooser owns the screen. Fresh candidates avoid stale schedule fallback on retry and preserve current disk drafts. No shared navigation API, endpoint, or server contract change. Existing show detection includes a just-started show whose intro has not aired yet and excludes completed historical shows.

Verification: 89 tests pass across Shows, Artist Station, navigation, AMA page, and draft persistence. Coverage includes currently playing and upcoming/hidden-filler show detection, excluding completed shows, locking selection during loading/failure, retry-to-forward, concurrent appearance calls, fresh checks on reappearance, preserving the parent stack, and ignoring late responses after Back/tab changes/task cancellation. Strict lint (364 files), format check, and diff whitespace check pass.
