...
| `exports/` | 2x PNG renders of every screen, grouped by proposal. Named `<screen-name>--<nodeId>.png`. |
...
2. **Look at it**: `Read` the PNG in `exports/<proposal>/` (agents can view
   images). This is the visual contract.
...
- **After any visual change to a screen, re-export it** so `exports/` never
  ...
  ```js
  Export(["<frameId>"], "png", "/Users/brian/playola/playola-radio-ios/design/exports/<proposal>")
  ```
