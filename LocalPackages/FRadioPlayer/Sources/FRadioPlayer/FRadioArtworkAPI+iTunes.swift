...
public struct iTunesAPI: FRadioArtworkAPI {
  ...
  public func getArtwork(
    for metadata: FRadioPlayer.Metadata, _ completion: @escaping (URL?) -> Void
  ) {

    guard !metadata.isEmpty, let rawValue = metadata.rawValue, let url = getURL(with: rawValue)
    else {
      completion(nil)
      return
    }
    ...
    let task = session.dataTask(with: url) { (data, response, error) in
      ...
      guard let response = try? JSONDecoder().decode(iTunesResponse.self, from: data),
        let artworkURL = response.results.first?.artworkURL(for: artworkSize)
      else {
        completion(nil)
        return
      }
