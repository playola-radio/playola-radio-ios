...
open class FRadioPlayer: NSObject {
  ...
  private override init() {
    ...
    // NOTE (Playola fork): the AVAudioSession setCategory call and the
    // interruption/route/headphone observers that lived here were removed.
    // The host app's AudioSessionCoordinator is the single owner of the
    // process-global AVAudioSession and of all interruption/route policy.
    // This player only does transport, metadata, artwork, and network-drop
    // recovery (reachability) — it never touches the session.

    // Reachability config
    try? reachability.startNotifier()
    NotificationCenter.default.addObserver(
      self, selector: #selector(reachabilityChanged(note:)), name: .reachabilityChanged,
      object: reachability)
  ...
  open func seek(to seconds: TimeInterval, completion: (() -> Void)?) {
    ...
    player?.seek(
      to: seekTime, toleranceBefore: .zero, toleranceAfter: .positiveInfinity,
  ...
  private func playerItemDidChange() {
    ...
    if let item = playerItem {
      NotificationCenter.default.addObserver(
        self, selector: #selector(itemDidPlayToEnd), name: .AVPlayerItemDidPlayToEndTime,
        object: playerItem)

      item.addObserver(
        self, forKeyPath: #keyPath(AVPlayerItem.status), options: [.old, .new],
        context: &playerItemContext)
      item.addObserver(
        self, forKeyPath: #keyPath(AVPlayerItem.isPlaybackBufferEmpty), options: [.old, .new],
        context: &playerItemContext)
      item.addObserver(
        self, forKeyPath: #keyPath(AVPlayerItem.isPlaybackLikelyToKeepUp), options: [.old, .new],
        context: &playerItemContext)
      item.addObserver(
        self, forKeyPath: #keyPath(AVPlayerItem.duration), options: [.old, .new],
        context: &playerItemContext)
  ...
  // NOTE (Playola fork): setupNotifications(), handleInterruption(), the route-
  // change handler, and checkHeadphonesConnection() were removed. Interruption
  // and route policy is owned solely by the host app's AudioSessionCoordinator.
  // Only reachability-based network-drop recovery remains below.
    ...
    if let item = object as? AVPlayerItem, let keyPath = keyPath, item == self.playerItem {
      ...
      case #keyPath(AVPlayerItem.status):
        ...
        if let statusNumber = change?[.newKey] as? NSNumber,
          let statusValue = AVPlayerItem.Status(rawValue: statusNumber.intValue)
        {
          status = statusValue
        } else {
          status = .unknown
...
extension FRadioPlayer: AVPlayerItemMetadataOutputPushDelegate {
  ...
  public func metadataOutput(
    ...
    didOutputTimedMetadataGroups groups: [AVTimedMetadataGroup], from track: AVPlayerItemTrack?
  ) {
